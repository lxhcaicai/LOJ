...
public class ProblemTestCaseUtils {
    ...
    public JSONObject tryInitTestCaseInfo(String testCaseDir,
                                          Long problemId,
                                          String version,
                                          String judgeMode,
        ...
        if(StringUtils.isEmpty(problemCases.get(0).getInput())
            ...
            if(FileUtil.isEmpty(new File(testCaseDir).getAbsoluteFile())) { //如果本地对应文件夹也为空，说明文件丢失了
                throw new SystemError("problemID:[" + problemId + "] test case has not found.", null, null);
            } else {
                return initLocalTestCase(judgeMode, judgeCaseMode, version, testCaseDir, problemCases);
            }
    ...
    public JSONObject initTestCase(List<HashMap<String,Object>> testCases,
        ...
        String testCaseDir = Constants.JudgeDir.TEST_CASE_DIR.getContent() + "/problem_" + problemId;
        ...
        for(int index = 0; index < testCases.size(); index ++) {
            ...
            // 生成对应的文件
            FileWriter infileWriter = new FileWriter(testCaseDir + "/" + inputName, CharsetUtil.UTF_8);
            // 将该测试数据的输入写入到文件
            ...
            if(Constants.JudgeCaseMode.DEFAULT.getMode().equals(judgeMode)) {
                // 原数据MD5
    ...
    public JSONObject initLocalTestCase(String judgeMode,
        ...
        result.set("judgeCaseMode", judgeCaseMode);
        result.set("version", version);
        result.set("testCaseSize", problemCaseList.size());
